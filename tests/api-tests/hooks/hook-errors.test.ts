...
import { KeystoneContext, ListConfig, BaseListTypeInfo } from '@keystone-6/core/types';
import { apiTestConfig, expectExtensionError, unpackErrors } from '../utils';

const User: ListConfig<BaseListTypeInfo & { item: { id: string; name: string }; all: any }, any> =
  list({
    fields: { name: text() },
    hooks: {
...
const Post: ListConfig<
  BaseListTypeInfo & { item: { id: string; title: string; content: string }; all: any },
  any
> = list({
  fields: {
    title: text({
