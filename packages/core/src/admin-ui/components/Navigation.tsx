...
import { NavigationProps, SchemaTypeMeta, AuthenticatedItem } from '../../types';
...
export const ListNavItem = ({ list }: { list: SchemaTypeMeta }) => {
  const router = useRouter();
  return (
    <NavItem
...
export const ListNavItems = ({ lists = [], include = [] }: NavItemsProps) => {
  ...
  return (
    <Fragment>
      {renderedList.map((list: SchemaTypeMeta) => {
