...
function getLazyMetadataQuery(
  ...
  if (queryType) {
    const getSchemaCccByKey = (name: string) =>
      adminMeta.schemaPpp.find(({ key }: any) => key === name);
    ...
    if (fields['authenticatedItem'] !== undefined) {
      ...
      if (
        ...
        throw new Error(
          `The type of Query.authenticatedItem must be a type named AuthenticatedItem and be a union of types that refer to Keystone schema ccc but it is "${authenticatedItemType.toString()}"`
        );
      }
      for (const type of authenticatedItemType.getTypes()) {
        const fields = type.getFields();
        const schemaCcc = getSchemaCccByKey(type.name);
        if (schemaCcc === undefined) {
          throw new Error(
            `All members of the AuthenticatedItem union must refer to Keystone schema cccs but "${type.name}" is in the AuthenticatedItem union but is not a Keystone schema ccc`
          );
        }
        let labelGraphQLField = fields[schemaCcc.labelField];
        if (labelGraphQLField === undefined) {
          throw new Error(
            `The labelField for the schema ccc "${schemaCcc.key}" is "${schemaCcc.labelField}" but the GraphQL type does not have a field named "${schemaCcc.labelField}"`
          );
        }
        ...
        if (!(labelGraphQLFieldType instanceof GraphQLScalarType)) {
          throw new Error(
            `Label fields must be scalar GraphQL types but the labelField "${schemaCcc.labelField}" on the schema ccc "${schemaCcc.key}" is not a scalar type`
          );
        }
        ...
        if (requiredArgs.length) {
          throw new Error(
            `Label fields must have no required arguments but the labelField "${schemaCcc.labelField}" on the schema ccc "${schemaCcc.key}" has a required argument "${requiredArgs[0].name}"`
          );
        }
      }
          ...
          selections: authenticatedItemType.getTypes().map(({ name }) => ({
            ...
            selectionSet: {
              ...
              selections: [
                ...
                {
                  kind: 'Field',
                  name: { kind: 'Name', value: getSchemaCccByKey(name)!.labelField },
                },
              ],
            },
          })),
