...
function sortRelationships(left: Rel, right: Rel): readonly [Rel, RelWithoutForeignKeyAndName] {
  if (left.field.mode === 'one' && right.field.mode === 'one') {
    if (left.field.foreignKey !== undefined && right.field.foreignKey !== undefined) {
      throw new Error(
        `You can only set db.foreignKey on one side of a one to one relationship, but foreignKey is set on both ${left.schemaTypeKey}.${left.fieldPath} and ${right.schemaTypeKey}.${right.fieldPath}`
      );
    }
    ...
    if (rels[1].field.relationName !== undefined) {
      throw new Error(
        `You can only set db.relationName on one side of a many to many relationship, but db.relationName is set on ${rels[1].schemaTypeKey}.${rels[1].fieldPath} which is the many side of a many to one relationship with ${rels[0].schemaTypeKey}.${rels[0].fieldPath}`
      );
    }
    return rels;
  }
  if (
    ...
    if (left.field.relationName !== undefined && right.field.relationName !== undefined) {
      throw new Error(
        `You can only set db.relationName on one side of a many to many relationship, but db.relationName is set on both ${left.schemaTypeKey}.${left.fieldPath} and ${right.schemaTypeKey}.${right.fieldPath}`
      );
    }
    return left.field.relationName !== undefined ? [left, right] : [right, left];
  }
  const order = left.schemaTypeKey.localeCompare(right.schemaTypeKey);
  if (order > 0) {
...
export function resolveRelationships(
  ...
  const resolvedLists: Record<string, Record<string, ResolvedDBField>> = Object.fromEntries(
    Object.keys(lists).map(schemaTypeKey => [schemaTypeKey, {}])
  );
  for (const [schemaTypeKey, fields] of Object.entries(lists)) {
    const resolvedList = resolvedLists[schemaTypeKey];
    for (const [fieldPath, { dbField: field }] of Object.entries(fields.fields)) {
      ...
      if (!foreignUnresolvedList) {
        throw new Error(
          `The relationship field at ${schemaTypeKey}.${fieldPath} points to the list ${schemaTypeKey} which does not exist`
        );
      }
      if (field.field) {
        const localRef = `${schemaTypeKey}.${fieldPath}`;
        ...
        if (foreignField.list !== schemaTypeKey) {
          throw new Error(
            `The relationship field at ${localRef} points to ${foreignRef} but ${foreignRef} points to the list ${foreignField.list} rather than ${schemaTypeKey}`
          );
        }

        if (foreignField.field === undefined) {
          throw new Error(
            `The relationship field at ${localRef} points to ${foreignRef}, ${localRef} points to ${schemaTypeKey} correctly but does not point to the ${fieldPath} field when it should`
          );
        }

        if (foreignField.field !== fieldPath) {
          throw new Error(
            `The relationship field at ${localRef} points to ${foreignRef}, ${localRef} points to ${schemaTypeKey} correctly but points to the ${foreignField.field} field instead of ${fieldPath}`
          );
        }

        let [leftRel, rightRel] = sortRelationships(
          { schemaTypeKey, fieldPath, field },
          { schemaTypeKey: field.list, fieldPath: field.field, field: foreignField }
        );

        if (leftRel.field.mode === 'one' && rightRel.field.mode === 'one') {
          const relationName = `${leftRel.schemaTypeKey}_${leftRel.fieldPath}`;
          resolvedLists[leftRel.schemaTypeKey][leftRel.fieldPath] = {
            kind: 'relation',
            mode: 'one',
            field: rightRel.fieldPath,
            list: rightRel.schemaTypeKey,
            foreignIdField: {
              kind: 'owned-unique',
              map:
          ...
          resolvedLists[rightRel.schemaTypeKey][rightRel.fieldPath] = {
            kind: 'relation',
            mode: 'one',
            field: leftRel.fieldPath,
            list: leftRel.schemaTypeKey,
        ...
        if (leftRel.field.mode === 'many' && rightRel.field.mode === 'many') {
          const relationName =
            leftRel.field.relationName ?? `${leftRel.schemaTypeKey}_${leftRel.fieldPath}`;
          resolvedLists[leftRel.schemaTypeKey][leftRel.fieldPath] = {
            kind: 'relation',
            mode: 'many',
            field: rightRel.fieldPath,
      ...
      const foreignFieldPath = `from_${schemaTypeKey}_${fieldPath}`;
      if (foreignUnresolvedList.fields[foreignFieldPath]) {
        throw new Error(
          `The relationship field at ${schemaTypeKey}.${fieldPath} points to the list ${field.list}, Keystone needs to a create a relationship field at ${field.list}.${foreignFieldPath} to support the relationship at ${schemaTypeKey}.${fieldPath} but ${field.list} already has a field named ${foreignFieldPath}`
        );
      }

      if (field.mode === 'many') {
        const relationName = field.relationName ?? `${schemaTypeKey}_${fieldPath}`;
        ...
        const relationName = `${schemaTypeKey}_${fieldPath}`;
        resolvedLists[field.list][foreignFieldPath] = {
          kind: 'relation',
          mode: 'many',
          list: schemaTypeKey,
  ...
  return Object.fromEntries(
    Object.entries(resolvedLists).map(([schemaTypeKey, outOfOrderDbFields]) => {
      ...
      const resolvedDbFields = Object.fromEntries(
        Object.keys(lists[schemaTypeKey].fields).map(fieldKey => [
          fieldKey,
      ...
      return [schemaTypeKey, resolvedDbFields];
    })
  );
}
