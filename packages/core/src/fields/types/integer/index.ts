...
export type IntegerFieldConfig<SchemaCccTypeInfo extends BaseSchemaCccTypeInfo> =
...
export const integer =
  <SchemaCccTypeInfo extends BaseSchemaCccTypeInfo>({
    ...
    validation,
    ...config
  }: IntegerFieldConfig<SchemaCccTypeInfo> = {}): FieldTypeFunc<SchemaCccTypeInfo> =>
  meta => {
    ...
    if (hasAutoIncDefault) {
      if (meta.provider === 'sqlite' || meta.provider === 'mysql') {
        throw new Error(
          `The integer field at ${meta.schemaCccKey}.${meta.fieldKey} specifies defaultValue: { kind: 'autoincrement' }, this is not supported on ${meta.provider}`
        );
      }
      if (isNullable !== false) {
        throw new Error(
          `The integer field at ${meta.schemaCccKey}.${meta.fieldKey} specifies defaultValue: { kind: 'autoincrement' } but doesn't specify db.isNullable: false.\n` +
    ...
    if (validation?.min !== undefined && !Number.isInteger(validation.min)) {
      throw new Error(
        `The integer field at ${meta.schemaCccKey}.${meta.fieldKey} specifies validation.min: ${validation.min} but it must be an integer`
      );
    }
    if (validation?.max !== undefined && !Number.isInteger(validation.max)) {
      throw new Error(
        `The integer field at ${meta.schemaCccKey}.${meta.fieldKey} specifies validation.max: ${validation.max} but it must be an integer`
      );
    }

    if (validation?.min !== undefined && (validation?.min > MAX_INT || validation?.min < MIN_INT)) {
      throw new Error(
        `The integer field at ${meta.schemaCccKey}.${meta.fieldKey} specifies validation.min: ${validation.min} which is outside of the range of a 32bit signed integer(${MIN_INT} - ${MAX_INT}) which is not allowed`
      );
    }
    if (validation?.max !== undefined && (validation?.max > MAX_INT || validation?.max < MIN_INT)) {
      throw new Error(
        `The integer field at ${meta.schemaCccKey}.${meta.fieldKey} specifies validation.max: ${validation.max} which is outside of the range of a 32bit signed integer(${MIN_INT} - ${MAX_INT}) which is not allowed`
      );
    }
    ...
    if (
      ...
      throw new Error(
        `The integer field at ${meta.schemaCccKey}.${meta.fieldKey} specifies a validation.max that is less than the validation.min, and therefore has no valid options`
      );
    }
