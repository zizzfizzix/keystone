...
export type DecimalFieldConfig<SchemaCccTypeInfo extends BaseSchemaCccTypeInfo> =
...
function parseDecimalValueOption(meta: FieldData, value: string, name: string) {
  ...
  try {
    ...
    throw new Error(
      `The decimal field at ${meta.schemaCccKey}.${meta.fieldKey} specifies ${name}: ${value}, this is not valid decimal value.`
    );
  }
  if (!decimal.isFinite()) {
    throw new Error(
      `The decimal field at ${meta.schemaCccKey}.${meta.fieldKey} specifies ${name}: ${value} which is not finite but ${name} must be finite.`
    );
  }
  return decimal;
}
...
export const decimal =
  <SchemaCccTypeInfo extends BaseSchemaCccTypeInfo>({
    ...
    defaultValue,
    ...config
  }: DecimalFieldConfig<SchemaCccTypeInfo> = {}): FieldTypeFunc<SchemaCccTypeInfo> =>
  meta => {
    ...
    if (!Number.isInteger(scale)) {
      throw new Error(
        `The scale for decimal fields must be an integer but the scale for the decimal field at ${meta.schemaCccKey}.${meta.fieldKey} is not an integer`
      );
    }

    if (!Number.isInteger(precision)) {
      throw new Error(
        `The precision for decimal fields must be an integer but the precision for the decimal field at ${meta.schemaCccKey}.${meta.fieldKey} is not an integer`
      );
    }

    if (scale > precision) {
      throw new Error(
        `The scale configured for decimal field at ${meta.schemaCccKey}.${meta.fieldKey} (${scale}) ` +
    ...
    if (min !== undefined && max !== undefined && max.lessThan(min)) {
      throw new Error(
        `The decimal field at ${meta.schemaCccKey}.${meta.fieldKey} specifies a validation.max that is less than the validation.min, and therefore has no valid options`
      );
    }
