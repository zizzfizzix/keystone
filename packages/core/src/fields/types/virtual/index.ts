...
export type VirtualFieldConfig<SchemaTypeTypeInfo extends BaseSchemaTypeTypeInfo> =
  CommonFieldConfig<SchemaTypeTypeInfo> & {
    field:
      | VirtualFieldGraphQLField<SchemaTypeTypeInfo['item']>
      | ((
...
export const virtual =
  ...
  meta => {
    ...
    if (
      ...
      throw new Error(
        `The virtual field at ${meta.schemaTypeKey}.${meta.fieldKey} requires a selection for the Admin UI but ui.query is unspecified and ui.listView.fieldMode and ui.itemView.fieldMode are not both set to 'hidden'.\n` +
          ...
          `query {\n` +
          `  ${
            getGqlNames({ schemaTypeKey: meta.schemaTypeKey, pluralGraphQLName: '' }).itemQueryName
          }(where: { id: "..." }) {\n` +
