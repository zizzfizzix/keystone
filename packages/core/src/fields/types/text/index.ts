...
export type TextFieldConfig<SchemaTypeTypeInfo extends BaseSchemaTypeTypeInfo> =
...
export const text =
  <SchemaTypeTypeInfo extends BaseSchemaTypeTypeInfo>({
    ...
    ...config
  }: TextFieldConfig<SchemaTypeTypeInfo> = {}): FieldTypeFunc<SchemaTypeTypeInfo> =>
  meta => {
    for (const type of ['min', 'max'] as const) {
      const val = _validation?.length?.[type];
      if (val !== undefined && (!Number.isInteger(val) || val < 0)) {
        throw new Error(
          `The text field at ${meta.schemaTypeKey}.${meta.fieldKey} specifies validation.length.${type}: ${val} but it must be a positive integer`
        );
      }
      if (_validation?.isRequired && val !== undefined && val === 0) {
        throw new Error(
          `The text field at ${meta.schemaTypeKey}.${meta.fieldKey} specifies validation.isRequired: true and validation.length.${type}: 0, this is not allowed because validation.isRequired implies at least a min length of 1`
        );
      }
    }
    ...
    if (
      ...
      throw new Error(
        `The text field at ${meta.schemaTypeKey}.${meta.fieldKey} specifies a validation.length.max that is less than the validation.length.min, and therefore has no valid options`
      );
    }
