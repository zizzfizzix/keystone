  ...
  meta => {
    ...
    const commonConfig = (
      ...
      if (values.size !== options.length) {
        throw new Error(
          `The select field at ${meta.schemaTypeKey}.${meta.fieldKey} has duplicate options, this is not allowed`
        );
      }
      return {
        ...config,
    ...
    if (config.type === 'integer') {
      if (
        ...
        throw new Error(
          `The select field at ${meta.schemaTypeKey}.${meta.fieldKey} specifies integer values that are outside the range of a 32 bit signed integer`
        );
      }
      return fieldType({
        kind: 'scalar',
    ...
    if (config.type === 'enum') {
      const enumName = `${meta.schemaTypeKey}${inflection.classify(meta.fieldKey)}Type`;
