...
export type SelectFieldConfig<SchemaCccTypeInfo extends BaseSchemaCccTypeInfo> =
...
export const select =
  <SchemaCccTypeInfo extends BaseSchemaCccTypeInfo>({
    ...
    validation,
    ...config
  }: SelectFieldConfig<SchemaCccTypeInfo>): FieldTypeFunc<SchemaCccTypeInfo> =>
  meta => {
    ...
    const commonConfig = (
      ...
      if (values.size !== options.length) {
        throw new Error(
          `The select field at ${meta.schemaCccKey}.${meta.fieldKey} has duplicate options, this is not allowed`
        );
      }
      return {
        ...config,
    ...
    if (config.type === 'integer') {
      if (
        ...
        throw new Error(
          `The select field at ${meta.schemaCccKey}.${meta.fieldKey} specifies integer values that are outside the range of a 32 bit signed integer`
        );
      }
      return fieldType({
        kind: 'scalar',
    ...
    if (config.type === 'enum') {
      const enumName = `${meta.schemaCccKey}${inflection.classify(meta.fieldKey)}Type`;
