...
export type BigIntFieldConfig<SchemaTypeTypeInfo extends BaseSchemaTypeTypeInfo> =
...
export const bigInt =
  <SchemaTypeTypeInfo extends BaseSchemaTypeTypeInfo>({
    ...
    ...config
  }: BigIntFieldConfig<SchemaTypeTypeInfo> = {}): FieldTypeFunc<SchemaTypeTypeInfo> =>
  meta => {
    ...
    if (hasAutoIncDefault) {
      if (meta.provider === 'sqlite' || meta.provider === 'mysql') {
        throw new Error(
          `The bigInt field at ${meta.schemaTypeKey}.${meta.fieldKey} specifies defaultValue: { kind: 'autoincrement' }, this is not supported on ${meta.provider}`
        );
      }
      if (isNullable !== false) {
        throw new Error(
          `The bigInt field at ${meta.schemaTypeKey}.${meta.fieldKey} specifies defaultValue: { kind: 'autoincrement' } but doesn't specify db.isNullable: false.\n` +
    ...
    for (const type of ['min', 'max'] as const) {
      if (validation[type] > MAX_INT || validation[type] < MIN_INT) {
        throw new Error(
          `The bigInt field at ${meta.schemaTypeKey}.${meta.fieldKey} specifies validation.${type}: ${validation[type]} which is outside of the range of a 64bit signed integer(${MIN_INT}n - ${MAX_INT}n) which is not allowed`
        );
      }
    }
    if (validation.min > validation.max) {
      throw new Error(
        `The bigInt field at ${meta.schemaTypeKey}.${meta.fieldKey} specifies a validation.max that is less than the validation.min, and therefore has no valid options`
      );
    }
