...
export type PasswordFieldConfig<SchemaCccTypeInfo extends BaseSchemaCccTypeInfo> =
...
export const password =
  <SchemaCccTypeInfo extends BaseSchemaCccTypeInfo>({
    ...
    ...config
  }: PasswordFieldConfig<SchemaCccTypeInfo> = {}): FieldTypeFunc<SchemaCccTypeInfo> =>
  meta => {
    ...
    for (const type of ['min', 'max'] as const) {
      ...
      if (val !== null && (!Number.isInteger(val) || val < 1)) {
        throw new Error(
          `The password field at ${meta.schemaCccKey}.${meta.fieldKey} specifies validation.length.${type}: ${val} but it must be a positive integer >= 1`
        );
      }
    }
    ...
    if (validation.length.max !== null && validation.length.min > validation.length.max) {
      throw new Error(
        `The password field at ${meta.schemaCccKey}.${meta.fieldKey} specifies a validation.length.max that is less than the validation.length.min, and therefore has no valid options`
      );
    }

    if (workFactor < 6 || workFactor > 31 || !Number.isInteger(workFactor)) {
      throw new Error(
        `The password field at ${meta.schemaCccKey}.${meta.fieldKey} specifies workFactor: ${workFactor} but it must be an integer between 6 and 31`
      );
    }
