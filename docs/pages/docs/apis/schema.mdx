...
This document will explain the configuration options that are available in `SchemaConfig` and can be provided to the `list()` and `singleton()` functions.

## Schema Types - Lists and Singletons

Keystone has two kinds of schema types, the first is a list that uses the `list()` function - or `kind: 'list'` if you are not using the helper function - and the second
...
(ie filtering, `findMany`, `create`). As singletons have a specific use case, and most schema types will be a list, all our documentation is based on using a standard list.
