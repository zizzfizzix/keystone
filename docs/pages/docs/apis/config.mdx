...
This lets you modify the node HTTP server that Keystone creates and runs and then runs on.
...
This function can be used to add a WebSocket server to the HTTP server for GraphQL subscriptions
...
_Note_: when running `keystone dev` `extendHttpServer` will only be run once on startup - changing this or anything that is referenced will not be pickup up by host reloading, you will need to stop and start Keystone manually
